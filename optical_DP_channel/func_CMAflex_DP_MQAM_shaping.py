...
def processing(mod, sps, SNR, nu, M_est, theta_diff,theta,lr_optim, batch_len, N_train_max, num_frames, flex_step, channel, symb_rate,tau_cd,tau_pmd,phiIQ, N_lrhalf):
    ...
    with torch.set_grad_enabled(False):  
        for frame in range(num_frames):
            ...
            out_const, h_est, e = sfun.CMAflex(rx_tensor, R, h_est, lr_optim, batch_len, flex_step, sps, True) 
            ...
            print(frame,'\t\ttraining: loss = ', torch.sum(e).item(), '\tshift_x = ',  shift[0].item(), '\tshift_y = ',  shift[1].item(), '\tr = ',  r  )    #, '\n h = ', h_est.tolist())
